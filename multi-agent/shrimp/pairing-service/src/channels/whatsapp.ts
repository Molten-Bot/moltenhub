...
let socketConnectionState: 'connecting' | 'open' | 'close' | null = null;
...
function getCredentialsPath(accountId: string = 'default'): string {
  const homeDir = process.env.HOME || '/home/moltenbot';
  return path.join(homeDir, '.openclaw', 'credentials', 'whatsapp', accountId);
}

export async function startWhatsAppPairing(session: PairingSession): Promise<void> {
  ...
  try {
    ...
    if (currentSessionId) {
      updateSession(currentSessionId, {
        status: 'error',
        error: `Failed to start WhatsApp: ${(error as Error).message}`,
      });
    }
    cleanupClient();
  } finally {
...
async function reconnectWithSavedCredentials(authDir: string): Promise<void> {
  ...
  try {
    ...
    if (currentSessionId) {
      updateSession(currentSessionId, {
        status: 'error',
        error: `Failed to reconnect: ${(error as Error).message}`,
      });
    }
    cleanupClient();
  }
