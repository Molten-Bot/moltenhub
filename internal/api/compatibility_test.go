...
func parseRouterRegisteredPaths(t *testing.T) []string {
	...
	for _, line := range strings.Split(string(content), "\n") {
		...
		path := rest[:end]
		if path == "/ping" || path == "/health" || path == "/openapi.yaml" || path == "/openapi.md" || path == "/docs" || strings.HasPrefix(path, "/v1/") {
			seen[path] = struct{}{}
		}
	}
...
func TestAPIModelContract_OnboardingAndOrganizationShape(t *testing.T) {
	...
	if gotURI, _ := humanObj["uri"].(string); gotURI != "https://hub.example.com/humans/alice" {
	...
	if gotURI, _ := orgObj["uri"].(string); gotURI != "https://hub.example.com/orgs/alpha-team" {
	...
	for _, item := range memberships {
		...
		if orgRow["org_id"] == orgID {
			...
			if gotURI, _ := orgRow["uri"].(string); gotURI != "https://hub.example.com/orgs/alpha-team" {
